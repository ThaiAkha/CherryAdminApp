...
const MarketShop: React.FC = () => {
  ...
  useEffect(() => {
    const loadMetadata = async () => {
      const meta = await contentService.getPageMetadata('admin-market-plan');
      if (meta) {
        setPageHeader(meta.titleMain || 'Market', meta.description || '');
      } else {
        setPageHeader('Market Planner', 'Plan daily purchases, approve requests, and sync with logistics.');
      }
    };
    loadMetadata();
  }, [setPageHeader]);
