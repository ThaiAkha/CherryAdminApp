...
const StoreFront: React.FC<{ onNavigate: (p: string) => void }> = ({ onNavigate: _onNavigate }) => {
    ...
    useEffect(() => {
        const loadMetadata = async () => {
            const meta = await contentService.getPageMetadata('store-front');
            if (meta) {
                setPageHeader(meta.titleMain || 'Store Front', meta.description || '');
            } else {
                setPageHeader('Store Front', 'Manage on-site sales, merchandise, and guest tabs during classes.');
            }
        };
        loadMetadata();
    }, [setPageHeader]);
