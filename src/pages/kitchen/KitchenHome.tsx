...
const KitchenHome: React.FC = () => {
    ...
    useEffect(() => {
        const loadMeta = async () => {
            const meta = await contentService.getPageMetadata('kitchen-home');
            if (meta) {
                setPageMeta(meta);
                setPageHeader(meta.titleMain || 'Kitchen', meta.description || '');
            } else {
                setPageHeader('Kitchen', 'Daily prep, bookings overview, and kitchen operations.');
            }
        };
        loadMeta();
    }, [setPageHeader]);
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-8 dark:border-gray-800 dark:bg-white/[0.03] md:p-12 relative overflow-hidden shadow-sm mb-8">
                <div className="relative z-10 space-y-6 max-w-2xl">
                    ...
                    <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tighter text-gray-800 dark:text-white leading-none">
                        {pageMeta?.titleMain || 'Kitchen'} <span className="text-brand-600">{pageMeta?.titleHighlight || 'Home'}</span>
                    </h1>
                    <p className="text-lg font-medium text-gray-500 dark:text-gray-400 max-w-lg leading-relaxed">
                        {pageMeta?.description || 'Daily prep, bookings overview, and kitchen operations.'}
                    </p>
                </div>
                {pageMeta?.imageUrl ? (
                    ...
                    <div className="absolute top-0 right-0 p-12 opacity-10 pointer-events-none">
                        <ChefHat className="w-64 h-64 text-brand-600" />
                    </div>
                )}
            </div>
        </PageContainer>
