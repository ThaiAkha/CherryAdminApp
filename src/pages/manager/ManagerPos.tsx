...
import { GridCard } from '../../components/data-explorer';
...
const ManagerPos: React.FC = () => {
    ...
    const [selectedSession, setSelectedSession] = useState<SessionType>('morning_class');
    ...
    useEffect(() => {
        const loadMetadata = async () => {
            const meta = await contentService.getPageMetadata('manager-pos');
            if (meta) {
                setPageHeader(meta.titleMain || 'Store POS', meta.description || '');
            } else {
                setPageHeader('Store POS', 'Manage shop sales, inventory, and guest tabs directly from the kitchen.');
            }
        };
        loadMetadata();
    }, [setPageHeader]);
            ...
            <PageContainer variant="full" className="h-[calc(100vh-64px)] flex flex-col">
                ...
                <PageGrid columns={12} className="flex-1 min-h-0 relative">
                    ...
                    <div className="lg:col-span-7 flex flex-col h-full bg-gray-50 dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden relative">
                        <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex flex-col gap-6">
                            ...
                            <div className="flex justify-center gap-3 overflow-x-auto no-scrollbar">
                                {mainCategories.map(cat => (
                                    <button
                                        ...
                                        className={cn(
                                            "flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold uppercase whitespace-nowrap transition-all border-2",
