...
const Badge: React.FC<{ type: string; value: string | number; isActive?: boolean }> = ({
    ...
    const baseStyles = "px-3 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full transition-colors";
...
const DataExplorerSidebar: React.FC<DataExplorerSidebarProps> = ({
    ...
    return (
        <div className="lg:col-span-2 flex flex-col bg-white dark:bg-[#0a0a0b] border-r border-gray-100 dark:border-white/[0.05] overflow-hidden">
            {/* Header */}
            <div className="h-16 px-5 border-b border-gray-100 dark:border-white/[0.05] bg-gray-50/30 dark:bg-white/[0.02] flex items-center justify-between shrink-0">
                ...
                {items.map((item) => {
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                "w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl border transition-all text-left group relative",
                                isActive
                                    ? "bg-orange-500/10 border-orange-500/20 shadow-[0_2px_10px_-4px_rgba(251,101,20,0.2)]"
                                    : "bg-transparent border-transparent hover:bg-orange-50 dark:hover:bg-orange-500/10"
                            )}
                        >
                            {/* Active Indicator Bar */}
                            {isActive && (
                                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-5 bg-orange-500 rounded-r-full shadow-[0_0_8px_rgba(251,101,20,0.5)]" />
                            )}
