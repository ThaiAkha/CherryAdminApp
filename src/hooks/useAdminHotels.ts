...
export function useAdminHotels() {
    ...
    useEffect(() => {
        const loadMeta = async () => {
            const meta = await contentService.getPageMetadata('admin-hotels');
            if (meta) {
                setPageHeader(meta.titleMain || 'Admin Hotels', meta.description || '');
            } else {
                setPageHeader('Admin Hotels', 'Manage hotel locations and pickup zones.');
            }
        };
        loadMeta();
        fetchData();
    }, [setPageHeader, fetchData]);
