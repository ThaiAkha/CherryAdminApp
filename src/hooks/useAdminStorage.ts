...
export const useAdminStorage = () => {
    ...
    // Load Metadata and Buckets
    useEffect(() => {
        const loadMeta = async () => {
            const meta = await contentService.getPageMetadata('admin-storage');
            if (meta) {
                setPageHeader(meta.titleMain || 'Admin Storage', meta.description || 'System buckets');
            } else {
                setPageHeader('Admin Storage', 'Manage system buckets and media assets.');
            }
        };
        loadMeta();
        fetchBuckets();
